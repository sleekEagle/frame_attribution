...
def calc_frame_shap_UCF101(GRP_PATH, OUT_PATH, FILL_METHOD, SHAP_METHOD, N_SAMPLES):
    #****************************************************************************
    # the model and the data loader
    #****************************************************************************
    ucf101dm = func.UCF101_data_model()
    ...
    #construct the out path for logging
    if SHAP_METHOD == 'exact':
        f = f'frame_{SHAP_METHOD}_'+ FILL_METHOD + '_' + Path(GRP_PATH).stem.split('_')[-1]+'.jsonl'
    else:
        f = f'frame_{SHAP_METHOD}_{N_SAMPLES}_'+ FILL_METHOD + '_' + Path(GRP_PATH).stem.split('_')[-1]+'.jsonl'
    # d = os.path.dirname(GRP_PATH)
    out_path = os.path.join(OUT_PATH,f)
